...
def test_scheduled_scraper_runs_in_process_when_aci_disabled(monkeypatch):
    ...
    def fake_run_pipeline(cfg, options):
        ...
        return type("Res", (), {"crawl_run_id": "x", "list_targets_attempted": 1, "list_pages_walked": 1, "posts_discovered": 1, "posts_skipped_existing": 0, "posts_processed": 1, "errors": []})()

    monkeypatch.setattr("function_app.run_pipeline", fake_run_pipeline)
    monkeypatch.setattr("function_app.pipeline_result_summary", lambda _res: {"ok": True})
...
def test_scheduled_scraper_dispatches_aci_when_enabled(monkeypatch):
    ...
    monkeypatch.setattr("blog_scraper.aci_invoke.aci_dispatcher_configured", lambda: True)
    seen: dict[str, object] = {}
...
def test_scheduled_scraper_skips_aci_when_not_configured(monkeypatch):
    monkeypatch.setenv("ACI_SCHEDULED", "true")
    monkeypatch.setattr("blog_scraper.aci_invoke.aci_dispatcher_configured", lambda: False)
